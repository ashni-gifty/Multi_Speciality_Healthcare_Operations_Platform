...
function MedicineStock() {
  ...
  useEffect(() => { pharmacyService.getMedicines().then((data) => setMedicines(Array.isArray(data) ? data : data.results || [])).catch((reason) => setError(reason.response?.data?.detail || "Unable to load medicine stock.")).finally(() => setLoading(false)); }, []);
