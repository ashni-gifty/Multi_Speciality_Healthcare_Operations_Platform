...
function PharmacyDashboard() {
  ...
  useEffect(() => {
    Promise.all([pharmacyService.getMedicines(), pharmacyService.getPrescriptions()])
      ...
      }).catch((reason) => setError(reason.response?.data?.detail || "Unable to load pharmacy dashboard data.")).finally(() => setLoading(false));
  }, []);
  ...
  return <main className="p-3 p-md-4 flex-grow-1">
